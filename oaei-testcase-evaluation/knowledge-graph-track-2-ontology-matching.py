...
TAR_PATH = "../testcases/knowledge-graph-2.7z"

SOURCE_OWL_PATH = load_ontology_from_7z(
    TAR_PATH,
    "knowledge-graph-2/starwars-source.xml"
)

TARGET_OWL_PATH = load_ontology_from_7z(
    TAR_PATH,
    "knowledge-graph-2/swtor-target.xml"
)

REF_ALIGNMENT_PATH = load_ontology_from_7z(
    TAR_PATH,
    "knowledge-graph-2/starwars-swtor-reference.xml"
)
...
def run_llm_alignment_with_ollama(source_contexts,target_contexts,top_matches,top_k,reference_alignments,path='starwars-swtor-mappings.csv'):
    """
    ...
    csv_path = os.path.join(os.path.dirname(__file__), '..', 'output\\knowledge-graph-2', path)
    ...
    print(f"\n LLM alignment results saved to {'output\\knowledge-graph-2',path}")
    return all_mappings
