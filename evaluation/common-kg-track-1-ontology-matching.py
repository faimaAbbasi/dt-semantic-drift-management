...
TAR_PATH = "../testcases/common-kg-1.7z"
...
def extract_relationship_triples(graph: Graph, properties: Set[str]) -> List[Tuple[str, str, str]]:
    """
    Generates Relationship Triples
    """
    triples = []
    for prop_uri in properties:
        prop = URIRef(prop_uri)
...
def run_llm_alignment_with_ollama(source_contexts,target_contexts,top_matches,top_k,reference_alignments,path='nel-dbpedia-mappings.csv'):
    """
    ...
    csv_path = os.path.join(os.path.dirname(__file__), '..', 'output\\common-kg-1', path)
    ...
    print(f"\n LLM alignment results saved to {'output\\common-kg-1',path}")
    return all_mappings
