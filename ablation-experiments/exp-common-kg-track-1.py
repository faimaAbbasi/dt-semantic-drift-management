...
TAR_PATH = os.path.join(os.path.dirname(__file__), '..', 'testcases', 'common-kg-1.7z')

SOURCE_OWL_PATH = load_ontology_from_7z(
    TAR_PATH,
    "common-kg-1/nel-source.rdf"
)

TARGET_OWL_PATH = load_ontology_from_7z(
    TAR_PATH,
    "common-kg-1/dbpedia-target.rdf"
)

REF_ALIGNMENT_PATH = load_ontology_from_7z(
    TAR_PATH,
    "common-kg-1/nel-dbpedia-reference.rdf"
)
...
def combined_similarity(
    ...
    config: str = "embeddings"
) -> float:
    """
    Compute similarity between two URIs using a named configuration.
    Uses rich embeddings (context-aware) as baseline; lexical and structure add complementary signals.
    """
    # Use rich embeddings (context-aware) instead of label-only embeddings
    #emb1 = rich_embs1[uri1]
    #emb2 = rich_embs2[uri2]
    ...
    if config == "embeddings+lexical":
        return 0.60 * emb_score + 0.30 * jw


    if config == "embeddings+lexical+structure":
        return 0.60 * emb_score + 0.20 * jw + 0.20 * jaccard
...
def run_llm_alignment_with_ollama(source_contexts,target_contexts,top_matches,top_k,reference_alignments,path='memoryalpha-stexpanded-mappings.csv'):
    """
    ...
    csv_path = os.path.join(os.path.dirname(__file__), '..', 'output\\knowledge-graph-1', path)
    ...
    print(f"\n LLM alignment results saved to {'output\\knowledge-graph-1',path}")
    return all_mappings
# ------------------------------------------
def evaluate_alignments(predicted_csv_path, reference_alignments, KNOWN_SOURCE_CLASSES, KNOWN_TARGET_CLASSES):
    predicted_alignments = set()
    
    for s_uri, top3 in predicted_csv_path.items():
        if s_uri in KNOWN_SOURCE_CLASSES:
            for t_uri, sim in top3:
                 predicted_alignments.add((s_uri, t_uri))
...
def main():
    ...
    #ref_set = set(reference_alignments)
    ...
    experiment_configs = [
        "embeddings",
        "embeddings+lexical",
        "embeddings+lexical+structure",
    ]

    # Note: Using rich_embs (context-aware) which already incorporate structure and semantic context
    ...
    for config_name in experiment_configs:
        ...
        eres = evaluate_alignments(results, reference_alignments, KNOWN_SOURCE_CLASSES, KNOWN_TARGET_CLASSES)
        metrics_rows.append({
            "configuration": config_name,
            "precision": eres["precision"],
            "recall": eres["recall"],
            "f1_score": eres["f1_score"],
        })
